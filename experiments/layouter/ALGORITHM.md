...
The current prototype realizes the crossing/track distinction with one provisional routing pass, one deterministic classification pass, and at most one reroute. It then performs two bounded route-improvement sweeps, a route-aware dock slide that preserves dock order, and two fixed shortest-path refinement sweeps of two passes each. Every eligible route is considered, including a collision-free fallback whose geometry is longer than necessary. Its local visibility grid contains at most 64 horizontal by 64 vertical coordinates: a bounded base set from the current route, nearby obstacle edges, allocated tracks, and the least common ancestor boundary, plus bounded must-keep subsets around crossed routes and adjacent bundle lanes. This visibility grid is the prototype realization of the relevant channel subgraph; direct traversal of canonical cell and portal identities remains the promotion target. Object intrusion, non-monotone container transit, immediate collinear reversal, and unauthorized positive-length overlap are hard invalidity conditions. The remaining score trades crossings against bends and length with one fixed crossing cost. Structured multi-region itineraries with multiple hard pins and monotone merge or bundle topology are not ripped up merely to shorten geometry.
