...
function parseArguments(argv) {
  const values = { entry: null, output: null, debugRouting: false };
  for (let index = 0; index < argv.length; index += 1) {
    if (argv[index] === "--output" || argv[index] === "-o") values.output = argv[++index];
    else if (argv[index] === "--debug-routing") values.debugRouting = true;
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const { entry, output, debugRouting } = parseArguments(process.argv.slice(2));
  if (!entry) {
    console.error("usage: node experiments/layouter/render.mjs <diagram.tsx> [--output preview.svg] [--debug-routing]");
    process.exit(2);
  }
